...
const TIME_SLOTS = ["12pm", "1pm", "2pm", "3pm", "4pm", "5pm", "6pm", "7pm", "8pm", "9pm", "10pm"]
...
export function WeeklyTimetable() {
  ...
  const loadWeekData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/timetable?start=${startDate}&end=${endDate}`)
      ...
      workshops.forEach((workshop: any) => {
        ...
        const timeSlot = hour >= 12 ? `${hour === 12 ? 12 : hour - 12}pm` : `${hour}am`
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-3xl font-bold text-foreground text-balance">Weekly Timetable</h2>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    <th className="p-3 text-left font-semibold text-sm sticky left-0 bg-muted/50 min-w-[80px]">TIME</th>
                    {DAYS.map((day, index) => (
                      <th key={day} className="p-3 text-center font-semibold text-sm min-w-[100px]">
                        <div>{day}</div>
                        {weekDates[index] && (
                        ...
                        return (
                          <td key={`${day}-${time}`} className="p-1.5">
                            {slot ? (
                              ...
                              >
                                <div className="text-sm font-medium text-foreground line-clamp-2">{slot.title}</div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex justify-between items-start gap-2 mb-1">
                                ...
                                <div className="text-sm text-muted-foreground whitespace-nowrap">{time}</div>
                              </div>
                              <div className="text-sm text-muted-foreground">{slot.instructor}</div>
                            </a>
                          )
                        })
                      ) : (
                        <div className="text-sm text-muted-foreground text-center py-4">No workshops scheduled</div>
                      )}
                    </div>
                  </div>
                )
      ...
      {isLoading && <div className="text-center text-muted-foreground py-8">Loading timetable...</div>}
    </div>
  )
}
