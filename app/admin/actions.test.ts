...
describe("createEvent", () => {
  ...
  it("should create weekly events until recurringUntil date when instructor sets recurring one month out", async () => {
    ...
    expect(lastEventDate.getTime()).toBeLessThanOrEqual(
      recurringUntil.getTime(),
    )
  })
})
...
describe("extendRecurringEvents", () => {
  ...
  it("should create events for series that need extension", async () => {
    ...
    expect(latestDate.getTime()).toBeGreaterThanOrEqual(
  ...
  it("should only extend if latest event still has isRecurring: true", async () => {
    ...
    if (firstNewEvent) {
      ...
      expect(firstNewEventDate.getTime()).toBeLessThanOrEqual(
        event2Date.getTime(),
      )
      ...
      const daysDiff = Math.abs(
        (firstNewEventDate.getTime() - event1Plus7.getTime()) /
          (1000 * 60 * 60 * 24),
      )
...
describe("deleteEventAndFutureInstructorEvents", () => {
  ...
  it("should send cancellation emails to all participants", async () => {
    ...
    await deleteEventAndFutureInstructorEvents(
      currentEvent.id,
      "Test cancellation message",
    )

    // Verify emails were sent
    expect(wasEmailSent("event-cancelled", "participant1@example.com")).toBe(
      true,
    )
    expect(wasEmailSent("event-cancelled", "participant2@example.com")).toBe(
      true,
    )

    const emails = getSentEmails()
    const email1 = emails.find(
      (e) =>
        e.type === "event-cancelled" && e.to === "participant1@example.com",
    )
    const email2 = emails.find(
      (e) =>
        e.type === "event-cancelled" && e.to === "participant2@example.com",
    )
  ...
  it("should fail when user is not admin or instructor", async () => {
    ...
    await expect(
      ...
      "Only admins and event instructors can delete events and all future instructor events",
    )
  })
  ...
  it("should fail when instructor tries to delete another instructor's events", async () => {
    ...
    await expect(
      ...
      "You can only delete events and future events you are instructing",
    )
  })
  ...
  it("should fail when event not found", async () => {
    ...
    await expect(deleteEventAndFutureInstructorEvents(99999)).rejects.toThrow(
      "Event not found",
    )
  })
  ...
  describe("recurring event update duplication bug", () => {
    it("should NOT create duplicate events when updating title of existing recurring event", async () => {
      ...
      expect(eventsBefore.map((e) => e.id)).toEqual(
    ...
    it("should NOT move second event to current date when editing first event in recurring series", async () => {
      ...
      const daysDiffAfter = Math.round(
        (secondDateAfter.getTime() - firstDateAfter.getTime()) /
          (1000 * 60 * 60 * 24),
      )
